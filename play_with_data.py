...
def train():
	...
	with open(train_filename) as train_file, open(test_filename) as test_file, open(output_filename, 'w') as out_file:
		...
		neigh = VotingClassifier(voting='hard',estimators=[
			('rf', RandomForestClassifier(n_estimators=400, n_jobs=-1)),
			('ada', AdaBoostClassifier(base_estimator=RandomForestClassifier(n_estimators=40, n_jobs=-1)))
			#('tree', AdaBoostClassifier(base_estimator=tree.DecisionTreeClassifier()))
			])
